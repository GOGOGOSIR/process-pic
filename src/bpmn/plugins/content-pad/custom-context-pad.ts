 ...
 */
export default function ContextPadProvider(this: any,
  config: any,
  injector: any,
  eventBus: any,
  contextPad: any,
...
ContextPadProvider.prototype.getContextPadEntries = function(element: any) {
  ...
  function getReplaceMenuPosition(element: any) {
    const Y_OFFSET = 5
  ...
  function appendAction(type: string, className: string, title?: string, options?: any) {
    ...
    const append = autoPlace
      ? function(event: any, element: any) {
        const shape = elementFactory.createShape(assign({ type }, options))
  ...
  function splitLaneHandler(count: any) {
...
function isEventType(eventBo: any, type: any, definition: any) {
  const isType = eventBo.$instanceOf(type)
  let isDefinition = false

  const definitions = eventBo.eventDefinitions || []
  forEach(definitions, (def: any) => {
